...
// 流式请求自动重试参数
const MAX_STREAM_RETRIES = 2; // 最大重试次数
const RETRY_DELAYS = [1000, 2500]; // 指数退避延迟 (ms)
...
// 流式响应处理相关类型定义
type StreamChunkProcessor = (chunk: any) => void;
type StreamErrorHandler = (error: Error) => void;
type StreamCompleteHandler = () => void;

interface StreamHandlers {
  onChunk: StreamChunkProcessor;
...
function processStreamChunk(chunk: string): any {
  try {
    ...
    console.error('处理流式数据块失败:', error, 'Raw chunk:', chunk);
    return null;
  }
}
  ...
  try {
    while (true) {
      ...
      if (done) {
        // 处理缓冲区中可能还有的最后一条数据
        if (buffer.trim()) {
          const chunk = processStreamChunk(buffer);
          if (chunk) {
      ...
      for (const line of lines) {
        if (line.trim()) {
          const chunk = processStreamChunk(line);
          if (chunk) {
            handlers.onChunk(chunk);
          }
        }
...
async function fetchWithStreamRetry(
  ...
  for (let attempt = 0; attempt <= MAX_STREAM_RETRIES; attempt++) {
    try {
      ...
      return; // 成功处理流式响应，退出函数
    } catch (error) {
      console.error(`流式请求失败 (尝试 ${attempt + 1}/${MAX_STREAM_RETRIES + 1}):`, error);
...
export async function fetchTimelineData(
  ...
  try {
    ...
    await new Promise<void>((resolve, reject) => {
          ...
          try {
            ...
            if (chunk.choices && chunk.choices[0] && chunk.choices[0].delta && chunk.choices[0].delta.content) {
              ...
              if (progressCallback && content.length > 10) {
                progressCallback('AI助手正在生成内容...', 'pending');
              }
            } else if (chunk.error) {
              // 处理错误信息
              console.error('流式请求返回错误:', chunk.error);
              streamError = new Error(chunk.error.message || '未知错误');
            }
          } catch (e) {
            console.error('处理流数据块失败:', e);
          }
        },
        onError: (error) => {
      ...
      // 执行流式请求，带自动重试
      fetchWithStreamRetry(apiUrl, requestOptions, handlers)
        .catch(error => {
          // 这是重试机制都失败的情况
          console.error('所有流式请求尝试均失败:', error);
          streamError = error;
          isCompleted = true;
          reject(error);
        });
    ...
    // 如果无内容但没有错误，尝试使用非流式请求作为后备方案
    if (!responseContent) {
      if (progressCallback) {
        progressCallback('流式请求无返回内容，尝试使用传统API请求', 'pending');
      }
    ...
    // 解析文本响应
    const result = parseTimelineText(responseContent);
